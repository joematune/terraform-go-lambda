...
data "archive_file" "lambda_hey_joe" {
  ...
  # TODO: Might be nice to add a script to not force naming the binary 'bootstrap'?
...
resource "aws_lambda_function" "hey_joe" {
  ...
  # References the function: https://docs.aws.amazon.com/lambda/latest/dg/golang-handler.html
  handler = "main"
  # In interpreted, we point to the file name, then dot, then exported function name = "hey.handler"
